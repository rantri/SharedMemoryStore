...
- [ ] T112 Freeze a second immutable candidate at new non-overwriting `009-final-r2-*` paths after T109-T111 pass. Preserve the original `009-final-linux-x64` pass and `009-final-pr` failure as diagnostic evidence only; rerun Linux x64, Windows PR, nightly, and release from one new clean commit/source manifest, import the new Linux report into release, and require every existing SC-001..SC-018, exact-tree, log-binding, completion-revalidation, dual-platform, and no-unresolved-high-severity-review predicate before approval.
