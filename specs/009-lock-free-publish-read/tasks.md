...
- [X] T104 Freeze one-shot PR/nightly/release execution at `009-final-r2-pr`, `009-final-r2-nightly`, and `009-final-r2-release` without overwriting preserved diagnostics or the rejected first final candidate; conditional closure requires matching Windows-x64/Linux-x64 schema-v3 evidence and identical clean commit/source-manifest provenance, entirely machine-derived with no tracked post-run edit
...
- [X] T114 Freeze the second immutable candidate at non-overwriting `009-final-r2-*` paths after T109-T113 pass. Preserve the original `009-final-linux-x64` pass and every rejected final/non-final diagnostic as evidence only. This checkbox freezes the Linux x64, Windows PR, nightly, and release commands and all SC-001..SC-018, exact-tree, log-binding, completion-revalidation, dual-platform, and no-unresolved-high-severity-review predicates; it does not assert that ignored final artifacts exist. After this tracked commit, approval is evidence-only at the exact T108 R2 paths, the Linux report must be imported untouched into release, every artifact must identify one clean commit/source manifest, and no tracked post-run edit is permitted.
