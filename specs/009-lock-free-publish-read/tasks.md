...
- [ ] T112 Preserve the rejected `009-r2-pre-final2-pr` diagnostic whose full suite and both churn tests passed but whose focused churn importer correctly rejected two rows against a stale one-row contract. Prove production is uninvolved; bind the configured churn step to the one exact case-sensitive FQN shared by both churn owner/leak mappings and present as one `[Fact]` in source; keep the fixed-key regression in the full suite; and make validation-only positive/negative cases reject distinct or missing mappings plus missing, extra-sibling, wrong, duplicate, or non-passed TRX rows. Pass PR/release validation-only checks, the exact focused churn test, the full solution, a new complete non-final PR diagnostic, and independent harness review without changing production or workload code.
- [ ] T113 Freeze a second immutable candidate at new non-overwriting `009-final-r2-*` paths after T109-T112 pass. Preserve the original `009-final-linux-x64` pass and every rejected final/non-final diagnostic as evidence only; rerun Linux x64, Windows PR, nightly, and release from one new clean commit/source manifest, import the new Linux report into release, and require every existing SC-001..SC-018, exact-tree, log-binding, completion-revalidation, dual-platform, and no-unresolved-high-severity-review predicate before approval.
...
| Performance and success criteria SC-001..SC-018 | T039-T041, T045-T046, T055, T073, T075-T113 |
