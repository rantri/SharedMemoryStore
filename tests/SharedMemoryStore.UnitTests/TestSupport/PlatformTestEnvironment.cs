...
internal static class PlatformTestEnvironment
{
    public static bool IsSupportedHost => OperatingSystem.IsWindows() || OperatingSystem.IsLinux();

    public static PlatformResourceName ResourceNameFor(string storeName)
    {
        return PlatformResourceName.Create(storeName);
    }

    public static IDisposable HoldStoreSynchronization(string storeName)
    {
        var resourceName = ResourceNameFor(storeName);
        ...
        throw new PlatformNotSupportedException("SharedMemoryStore test synchronization is available on Linux and Windows.");
    }
